...
public class ReiMinimap implements Runnable {
    ...
    private static final EmptyChunk EMPTY_CHUNK;
        ...
        try {
            ...
            if (this.theWorld != this.theMinecraft.theWorld) {
                ...
                if (this.theWorld != null) {
                    Environment.setWorldSeed(this.theWorld.getRandomSeed());
                    boolean changeWorld;
                    String levelName;
                    if (this.multiplayer) {
                        levelName = null;
                        ...
                        levelName = (String)getFields(this.theWorld, "worldInfo", "levelName");
                        if (levelName == null) {
                    ...
                    Integer dimension = (Integer)getField(this.thePlayer, "dimension");
                    if (dimension == null) {
            ...
            if (!this.chatWelcomed && System.currentTimeMillis() < this.chatTime + 10000L) {
                ...
                while(i$.hasNext()) {
                    ...
                    Matcher matcher = Pattern.compile("\u00a70\u00a70((?:\u00a7[1-9a-d])+)\u00a7e\u00a7f").matcher(cl.message);
            ...
            this.scaledResolution = new ScaledResolution(this.theMinecraft.gameSettings, displayWidth, displayHeight);
            ...
            if (this.deathPoint && this.theMinecraft.currentScreen instanceof GuiGameOver && !(this.guiScreen instanceof GuiGameOver)) {
                ...
                x = MathHelper.floor_double(this.thePlayer.posX);
                x = MathHelper.floor_double(this.thePlayer.posY);
                y = MathHelper.floor_double(this.thePlayer.posZ);
            ...
            if (this.texture.getId() != 0) {
                ...
                case 1:
                    ...
                    y -= scale * ((this.showMenuKey | this.showCoordinate ? 2 : 0) + (this.showMenuKey ? 9 : 0) + (this.showCoordinate ? 18 : 0)) / this.scaledResolution.scaleFactor;
                    break;
                case 2:
                default:
                    x = this.scWidth - 37;
                ...
                int temp = this.scaledResolution.scaleFactor + 1 >> 1;
                scalef = 1.0F / (float)this.scaledResolution.scaleFactor * (float)temp;
                GL11.glPushMatrix();
                ...
                if (!this.fullmap && alpha > 0) {
                    ...
                    width = mc.fontRenderer.getStringWidth(str);
                    if (alpha > 255) {
                        alpha = 255;
                    }

                    _x = (int)((float)(x + 30) / scalef - (float)width);
                    _y = (int)((float)(y + 26) / scalef);
                    int argb = alpha << 24 | 16777215;
                    mc.fontRenderer.drawStringWithShadow(str, _x, _y, argb);
                }

                if (this.showCoordinate) {
                    int posX = MathHelper.floor_double(this.thePlayer.posX);
                    width = MathHelper.floor_double(this.thePlayer.boundingBox.minY);
                    _x = MathHelper.floor_double(this.thePlayer.posZ);
                    _x = (int) ((this.fullmap ? (float)this.scWidth * 0.5F : (float)x) / scalef);
                    _y = (int) ((this.fullmap ? (float)this.scHeight * 0.5F + 16.0F : (float)(y + 32)) / scalef);
                    str = String.format("%+d, %+d", posX, _x);
                    width = (int) ((float)mc.fontRenderer.getStringWidth(str) * 0.5F);
                    mc.fontRenderer.drawStringWithShadow(str, (int)(_x - width), (int)(_y + 2.0F), 16777215);
                    str = Integer.toString(width);
                    width = (int) ((float)mc.fontRenderer.getStringWidth(str) * 0.5F);
                    mc.fontRenderer.drawStringWithShadow(str, (int)(_x - width), (int)(_y + 11.0F), 16777215);
                }

                if (this.showMenuKey && !this.fullmap) {
                    str = String.format("Menu: %s key", KeyInput.MENU_KEY.getKeyName());
                    width = this.theMinecraft.fontRenderer.getStringWidth(str);
                    _x = (int)((float)(x + 32) / scalef - (float)width);
                    _y = (int)((float)(y + 32) / scalef);
                    this.theMinecraft.fontRenderer.drawStringWithShadow(str, _x, _y + (this.showCoordinate ? 20 : 2), -1);
                }
    ...
    private void mapCalc(boolean strip) {
        if (this.theWorld != null && this.thePlayer != null) {
            ...
            if (this.stripCounter.count() == 0) {
                ...
                this.skylightSubtracted = this.theWorld.skylightSubtracted;
                d = Math.toRadians(this.roundmap && !this.fullmap ? (double)(45.0F - this.thePlayer.rotationYaw) : -45.0D);
    ...
    private void surfaceCalc(Chunk chunk) {
        if (chunk != null && !(chunk instanceof EmptyChunk)) {
            ...
            boolean slime = this.showSlimeChunk && this.currentDimension == 0 && this.chunkCache.isSlimeSpawn(chunk.xPosition, chunk.zPosition);
            ...
            Chunk cpz = null;
            if (this.undulate) {
                chunkMinusZ = this.getChunk(chunk.worldObj, chunk.xPosition, chunk.zPosition - 1);
                chunkPlusZ = this.getChunk(chunk.worldObj, chunk.xPosition, chunk.zPosition + 1);
                chunkMinusX = this.getChunk(chunk.worldObj, chunk.xPosition - 1, chunk.zPosition);
                chunkPlusX = this.getChunk(chunk.worldObj, chunk.xPosition + 1, chunk.zPosition);
            }
                ...
                if (zCoord >= 0) {
                    ...
                    for(int x = 0; x < 16; ++x) {
                        ...
                        if (xCoord >= 0) {
                            ...
                            pixel.clear();
                            int height = !this.omitHeightCalc && !this.heightmap && !this.undulate ? 127 : Math.min(127, chunk.getHeightValue(x, z));
                            int y = this.omitHeightCalc ? height : 127;
                            this.surfaceCalc(chunk, x, y, z, pixel, (TintType)null);
                            float factor;
                            if (this.heightmap) {
                                factor = this.undulate ? 0.25F : 0.6F;
                                double d1 = (double)(height - this.posY);
                            ...
                            if (this.undulate) {
                                ...
                                int mx = cmx.getHeightValue(x - 1 & 15, z);
                                int px = cpx.getHeightValue(x + 1 & 15, z);
                                int mz = cmz.getHeightValue(x, z - 1 & 15);
                                int pz = cpz.getHeightValue(x, z + 1 & 15);
    ...
    private void surfaceCalc(Chunk chunk, int x, int y, int z, PixelColor pixel, TintType tintType) {
        ...
        if (blockID == 0 || this.hideSnow && blockID == 78) {
            ...
            case 1:
                lightValue = y < 127 ? chunk.getBlockLightValue(x, y + 1, z, 0) : 15;
                break;
            case 2:
                lightValue = y < 127 ? chunk.getBlockLightValue(x, y + 1, z, 11) : 4;
                break;
            case 3:
                lightValue = 15;
                break;
                        ...
                        if (xCoord >= 0) {
                            ...
                            case -1:
                                ...
                                while(true) {
                                    ...
                                    _y = this.posY - y;
                                    if (_y >= 0 && _y < 128 && chunk.getBlockID(x, _y, z) == 0 && chunk.getBlockLightValue(x, _y, z, 12) != 0) {
                                        f += temp[y];
                                    }
                            ...
                            case 0:
                                for(y = 0; y < temp.length; ++y) {
                                    _y = this.posY - y;
                                    if (_y >= 128 || _y >= 0 && chunk.getBlockID(x, _y, z) == 0 && chunk.getBlockLightValue(x, _y, z, 12) != 0) {
                                        f += temp[y];
                                    }
    ...
    private void renderRoundMap(int x, int y) {
        ...
        double slideX = (this.thePlayer.posX - (double)this.lastX) * 0.00390625D;
        double slideY = (this.thePlayer.posZ - (double)this.lastZ) * 0.00390625D;
        ...
        if (this.visibleEntitiesRadar) {
            List<Entity> entityList = this.theWorld.loadedEntityList;
            synchronized(entityList) {
                ...
                while(i$.hasNext()) {
                    ...
                    if (color != 0) {
                        double wayX = this.thePlayer.posX - entity.posX;
                        distance = this.thePlayer.posZ - entity.posZ;
                        ...
                        try {
                            ...
                            if (distance < 29.0D) {
                                ...
                                float alpha = (float)Math.max(0.20000000298023224D, 1.0D - Math.abs(this.thePlayer.posY - entity.posY) * 0.04D);
                                GL11.glColor4f(r, g, b, alpha);
                                GL11.glTranslatef((float)x, (float)y, 0.0F);
                                GL11.glRotatef(-locate - this.thePlayer.rotationYaw + 180.0F, 0.0F, 0.0F, 1.0F);
                                ...
                                if (this.configEntityDirection) {
                                    GL11.glTranslatef((float)x, (float)y, 0.0F);
                                    GL11.glRotatef(entity.rotationYaw - this.thePlayer.rotationYaw, 0.0F, 0.0F, 1.0F);
                                    GL11.glTranslatef((float)(-x), (float)(-y), 0.0F);
        ...
        if (this.visibleWaypoints) {
            ...
            while(i$.hasNext()) {
                ...
                if (pt.enable) {
                    wayX = this.thePlayer.posX - (double)pt.x - 0.5D;
                    double wayZ = this.thePlayer.posZ - (double)pt.z - 0.5D;
        ...
        double s = Math.sin(Math.toRadians((double)this.thePlayer.rotationYaw)) * 28.0D;
        wayX = Math.cos(Math.toRadians((double)this.thePlayer.rotationYaw)) * 28.0D;
    ...
    private void renderSquareMap(int x, int y) {
        ...
        if (this.visibleEntitiesRadar) {
            ...
            synchronized(entityList) {
                ...
                while(i$.hasNext()) {
                    ...
                    if (color != 0) {
                        double wayX = this.thePlayer.posX - entity.posX;
                        wayZ = this.thePlayer.posZ - entity.posZ;
                        ...
                        try {
                            ...
                            if (t < 31.0D) {
                                ...
                                if (this.configEntityDirection) {
                                    ...
                                    GL11.glRotatef(entity.rotationYaw - 90.0F, 0.0F, 0.0F, 1.0F);
                                    GL11.glTranslated((double)(-x) - wayZ, (double)(-y) + wayX, 0.0D);
                                    this.texture("%blur%/reifnsk/minimap/entity2.png");
        ...
        if (this.visibleWaypoints) {
            ...
            while(i$.hasNext()) {
                Waypoint pt = (Waypoint)i$.next();
                if (pt.enable) {
                    double wayX = this.thePlayer.posX - (double)pt.x - 0.5D;
                    wayZ = this.thePlayer.posZ - (double)pt.z - 0.5D;
                    wayX = wayX * this.currentZoom * 0.5D;
                    wayZ = wayZ * this.currentZoom * 0.5D;
                    float locate = (float)Math.toDegrees(Math.atan2(wayX, wayZ));
        ...
        try {
            ...
            GL11.glRotatef(this.thePlayer.rotationYaw - 90.0F, 0.0F, 0.0F, 1.0F);
            GL11.glTranslated((double)(-x), (double)(-y), 0.0D);
        ...
        if (this.visibleEntitiesRadar) {
            ...
            synchronized(entityList) {
                ...
                while(i$.hasNext()) {
                    ...
                    if (color != 0) {
                        double wayX = this.thePlayer.posX - entity.posX;
                        d = this.thePlayer.posZ - entity.posZ;
                    ...
                    try {
                        ...
                        if (d < 114.0D) {
                            ...
                            if (KeyInput.TOGGLE_ZOOM.isKeyDown() && pt.name != null && !pt.name.isEmpty()) {
                                ...
                                int width = this.theMinecraft.fontRenderer.getStringWidth(pt.name);
    ...
    private int getDimension() {
        return this.theMinecraft.thePlayer.dimension;
    }
    ...
    static {
        ...
        EMPTY_CHUNK = new EmptyChunk((World)null, EMPTY_CHUNK_BYTES, 0, 0);
